...
shore_crane = load_gsheet_data(TRANSPORT_SHEET_ID, shore_crane_sheet).select(
...
transfer = (
    ...
    .join_asof(TRANSFER_PRICE, by="Service", right_on="Date",left_on="date", strategy="backward")
    .with_columns(
...
scow_transfer:pl.LazyFrame= load_gsheet_data(TRANSPORT_SHEET_ID, scow_sheet).select(
    pl.col("date"),
    ...
    .with_columns(
        overtime_150=pl.when(
            (pl.col("day").is_in(SPECIAL_DAYS).not_()).and_(
                (pl.col("end_time").gt(UPPER_BOUND))
